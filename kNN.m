...
L = length(vzor); % celkový poèet vzorù

% výpoèet euklidovské vzdálenosti všech vzorù od zadaneho bodu
for i=1:L
    euler(i) = sqrt(sum((vzor(i,:) - bod).^2));
end
nejmensi_vzdalenost=min(euler);

a=0;    % indexace vektoru "pozice"
for i=1:L
    if euler(i) == nejmensi_vzdalenost  % pokud je na pozici "i" stejna hodnota jako nejmensi vzdalenost 
        a=a+1;
