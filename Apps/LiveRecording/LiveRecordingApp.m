classdef LiveRecordingApp < handle
%
% Usage:
%     app.LiveRecordingApp

  properties
      AppPath = {'/LiveRecording'};
	  AppClass = 'LiveRecordingApp';
	  AppHandle;
	  AppCount = 0;
      Increment = 1;
	  Decrement = 0;
  ...
  methods
    ...
    function startApp(obj)
        ...
        if nargout(@LiveRecording) == 0  
            eval('LiveRecording');
        else
			obj.AppHandle = eval('LiveRecording');   
        end 
        % Increment the reference count by one    
        LiveRecordingApp.refcount(obj.Increment);
